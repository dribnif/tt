...
def action_report(colorizer, activity=None):
    ...
    work = data['work']
    report = defaultdict(
        lambda: {'sum': timedelta(), 'notes': defaultdict(), 'weekday': '', 'start_time': None, 'end_time': None})
    ...
    for item in work:
        if (item['name'] == activity or activity is None) and 'end' in item:
            ...
            prev_notes = report[day]['notes'].get( item['name'], '')
            report[day]['notes'][item['name']] = prev_notes + reportingutils.get_notes_from_workitem(item)
